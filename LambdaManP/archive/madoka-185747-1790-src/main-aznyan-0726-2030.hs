...
(dirValuePill:: Expr Int -> Expr Pos -> Expr [[Int]] -> Expr Pos -> Expr Int, dirValuePillDef) =
  def4 "dirValuePill" $ \depth vect chizu manp ->
    let info = (mapAt manp chizu) 
        subScore = (dirValuePill (depth+1) (vrotR vect) chizu manp
                 + dirValuePill (depth+1) (vrotL vect) chizu manp) `div` 2
    in 
    ite (depth .>= 3) 0 $
    ite (info .== 0) subScore $
    (tileValue info) + (dirValuePill depth vect chizu $ vadd manp vect)*9`div`10
...
(dirValueTotal:: Expr Pos -> Expr World -> Expr Int, dirValueTotalDef) =
  def2 "dirValueTotal" $ \vect world -> 
    ...
    in 
        ...
        dirValuePill 0 vect chizu manP + 
