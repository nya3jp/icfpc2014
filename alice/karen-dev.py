...
def main(world, ghosts):
  ...
  return (state, make_progress)

def make_progress(state, world):
  cycle = car(state)
...
def decide(tactics, globals):
  ...
  if tactics == 2:
    dir = expiring_decide(globals)
  if tactics == 3:
    dir = dinner_decide(globals)
  return tactics, dir

def maybe_change_tactics(tactics, globals):
  vitality = get_vitality(globals)
  if tactics == 2:
    if vitality > 0:
      return tactics
    tactics = 0
...
def compute_safety(globals):
  ...
  ghosts = get_normal_ghosts(globals)
  for t in range(5):
...
def classic_eval_cell(x, y, dist, globals):
  for ghost in get_simple_ghosts(globals):
    gx, gy = unpair(car(ghost))
    if x == gx and y == gy:
      return -10000
...
def get_normal_ghosts(globals):
  ghosts = []
  for gentry in get_full_ghosts(globals):
...
def expiring_decide(globals):
  print 987654321
  unghosts = reverse_players(get_simple_ghosts(globals), globals)
  unghost_map = update_player_map(get_white(globals), unghosts)
  unghosts = simulate_player_move(unghosts, globals)
  unghost_map = update_player_map(unghost_map, unghosts)
  unghosts = simulate_player_move(unghosts, globals)
  unghost_map = update_player_map(unghost_map, unghosts)
  cx, cy, cdir = get_self_once(globals)
  best_score, best_dir = generic_priority_search(cx, cy, -1, 3, unghost_map)
  return best_dir

def reverse_players(players, globals):
  map = get_map(globals)
  reversed_players = []
  for player in players:
    cx = car(car(player))
    ...
    cdir = cdr(player)
    rdir = (cdir + 2) % 4
    if get_map_cell(map, adj(cx, cy, rdir)) == 0:
      rdir = (rdir + 1) % 4
      if get_map_cell(map, adj(cx, cy, rdir)) == 0:
        rdir = (rdir + 2) % 4
        if get_map_cell(map, adj(cx, cy, rdir)) == 0:
          rdir = (rdir + 3) % 4
    reversed_players = (((cx, cy), rdir), reversed_players)
  return reversed_players

@rank(2)
def generic_priority_search(cx, cy, cdir, limit, map):
  base_score = get_map_cell(map, cx, cy)
  if limit == 0:
    return base_score, -1
  choices = []
  for ndir in range(4):
    if cdir >= 0 and ndir == (cdir + 2) % 4:
      continue
    if get_map_cell(map, adj(cx, cy, ndir)) >= 0:
      choices = (ndir, choices)  # prepend
  if atom(choices):
  ...
  best_dir = -1
  for ndir in choices:
    score, _ = generic_priority_search(adj(cx, cy, ndir), ndir, limit - 1, map)
    if score > best_score:
      best_score = score
      best_dir = ndir
  return base_score + best_score, best_dir
...
def dinner_decide(globals):
  ox, oy, _ = get_self_once(globals)
  ghost_map = update_player_map(get_simple_ghosts(globals))
  current_selfs = []
  for dir in xrange(4):
    current_selfs = (, current_selfs)
  for 
  lambdas = [
  ghosts = 
...
@nolocals
def get_full_ghosts(globals):
  return cdr(cdr(car(globals)))

def get_simple_ghosts(globals):
  ghosts = []
  for gentry in cdr(cdr(car(globals))):
    ghosts = (cdr(gentry), ghosts)
  return ghosts
